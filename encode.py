...
def process_file(file):
    ...
    # Process the next two rows to finds tents in each row and column
    for row_index, n_tents in enumerate(lines[dim].split(" ")):
        rows.append((row_index, n_tents))
    for col_index, n_tents in enumerate(lines[dim+1].split(" ")):
